...
def link_action_configs(
    ...
    for builtin_library_directory in builtin_library_directories:
        builtin_library_directory_link_flags += ["/libpath:{}".format(builtin_library_directory)]

    builtin_library_link_flags = []
    for builtin_library in builtin_libraries:
        builtin_library_link_flags += [builtin_library]
    ...
    for builtin_executable_object in builtin_executable_objects:
        builtin_executable_objects_link_flags += [builtin_executable_object]
    ...
    return [
        action_config(
            ...
            tools = [tool(path = "{}/bin/lld-link".format(binutils))],
            flag_sets = [
